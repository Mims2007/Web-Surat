                            ...
                            <div class="z-depth-1">
                                <nav class="secondary-nav">
                                    <div class="nav-wrapper blue-grey darken-1">
                                        <div class="col 12">
                                            <ul class="left">
                                                <li class="waves-effect waves-light"><a href="?page=gsm" class="judul"><i class="material-icons">menu_book</i>Galeri file<a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </nav>
                ...
                if($dari_tanggal == "" || $sampai_tanggal == ""){
                    ...
                    if(mysqli_num_rows($query) > 0){
                        while($row = mysqli_fetch_array($query)){
                            echo '<tr>
                                    <td><a href="./upload/surat_masuk/'.$row['file'].'" target="_blank">'.$row['file'].'</a></td>
                                    <td>'.$row['no_surat'].'</td>
                                    <td>'.indoDate($row['tgl_diterima']).'</td>
                                    <td><a class="btn light-green darken-1" href="?page=gsm&act=fsm&id_surat='.$row['id_surat'].'">Lihat Detail File</a></td>
                                  </tr>';
                        }
                    } else {
